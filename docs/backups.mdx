...
Workflow использует существующий production SSH-доступ и требует, чтобы deploy-пользователь мог выполнить `sudo` без запроса пароля. Он не передаёт в GitHub ни пароль Яндекс Диска, ни пароль шифрования rclone.
